...
Replace mode substitutes each detected entity with a replacement token. Anonymizer provides four strategies. 
...
| `format_template` | `<HASH_{label}_{digest}>` | Template with `{digest}` required and optional `{label}`. |

---

## Substitute

Replaces entities with LLM-generated synthetic values that are contextually plausible. This is the only strategy that requires an LLM call.
