...
class AugmentedEntitySchema(BaseModel):
    # No min_length: a single empty value/label must not fail-validate the whole
    # augmentation batch on a small model. apply_augmented_entities() skips
    # blank value/label downstream, so empties are dropped without losing the row.
    value: str = ""
    label: str = ""
    reason: str | None = None
