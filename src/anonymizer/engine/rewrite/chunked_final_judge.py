...
The final judge scores a rewrite on privacy / quality / style. Normally it
...
display/extract code is unchanged: ``{rubric_name: {"score": str, "reasoning": str}}``
with scores drawn from the rubric options (``high`` / ``medium`` / ``low``).
...
_DIMENSIONS = ("privacy", "quality", "style")

# Ordering for worst-window aggregation: lower rank = worse.
...
def aggregate_judge(results: list[JudgeScoresSchema]) -> dict[str, dict[str, Any]]:
    ...
    Per dimension, takes the worst score across windows (the worst section drives
    the score) and the reasoning from that worst window. Shape matches
    LLMJudgeColumnConfig output: ``{dim: {"score": str, "reasoning": str}}``.
    """
    out: dict[str, dict[str, Any]] = {}
    for dim in _DIMENSIONS:
        dims = [getattr(r, dim) for r in results]
        worst = min(dims, key=lambda d: _SCORE_RANK[d.score])
