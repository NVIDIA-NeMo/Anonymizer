...
def _validate_complete_revisions(expected: tuple[object, ...], revisions: object) -> bool:
    return (
        type(revisions) is dict
        and _has_exact_keys(expected, tuple(revisions))
        and all(type(value) is str for value in revisions.values())
    )
...
def _evaluate_metrics(
    ...
    *,
    ...
    if not all(isfinite(score) and 0 <= score <= 1 for _, score in utility) or not all(
        isfinite(confidence) and 0 <= confidence <= 1 for _, confidence, _ in privacy
    ):
        return None
    ...
    utility_score = (
        sum(weight * score for weight, score in utility) / sum(weight for weight, _ in utility)
        if utility
        else (1.0 if exact_baseline else 0.0)
    )
    high = any(sensitivity == "high" and yes for sensitivity, _, yes in privacy)
    return _Phase8Metric(
        utility_score,
        leakage,
        leakage / denominator if denominator else 0.0,
        high,
        (high if repair_any_high else leakage > repair_threshold) or utility_score < utility_floor,
    )
