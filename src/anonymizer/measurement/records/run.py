...
def _detect_config_metadata(detect: Any | None) -> dict[str, Any]:
    ...
    if entity_labels is None:
        ...
        configured_labels = list(DEFAULT_ENTITY_LABELS)
    else:
        entity_label_count = len(entity_labels)
        configured_labels = list(entity_labels)
    excluded_entity_labels = getattr(detect, "excluded_entity_labels", None)
    excluded = set(excluded_entity_labels or [])
    active_entity_labels = [label for label in configured_labels if label not in excluded]
    return {
        ...
        "excluded_entity_labels": list(excluded_entity_labels) if excluded_entity_labels is not None else None,
        "effective_entity_labels": active_entity_labels,
        "effective_entity_label_count": len(active_entity_labels),
