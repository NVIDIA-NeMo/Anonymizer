...
- **Self-hosted GLiNER2:** Use `create_anonymizer(gliner=NativeGliner(device="auto"))` for the library-managed singleton runtime. Use `create_anonymizer(gliner=GlinerEndpoint(url=..., model=..., api_key_env=...))` for an already-managed compatible endpoint. The endpoint path preserves unrelated model configs, providers, and selected roles; it does not start or stop the service. For a source-checkout vLLM deployment, compile and launch with `tools/inference_service.py`, then read non-secret client values with its `connection` command. See [`docs/concepts/self-hosting-gliner.md`](../../docs/concepts/self-hosting-gliner.md) or the [published self-hosting guide](https://nvidia-nemo.github.io/Anonymizer/dev/concepts/self-hosting-gliner/).
...
- **Local / on-prem GLiNER2:** For native development, install the notebook extra and pass `NativeGliner(device="auto")` to the public factory. For an existing service, pass its URL, served model, and credential environment-variable name through `GlinerEndpoint`. Do not call `stop_local_runtime()` for external endpoints; their launcher owns shutdown. A wrong endpoint or stopped service fails when detection sends its request. See [`docs/concepts/self-hosting-gliner.md`](../../docs/concepts/self-hosting-gliner.md) or the [published self-hosting guide](https://nvidia-nemo.github.io/Anonymizer/dev/concepts/self-hosting-gliner/).
