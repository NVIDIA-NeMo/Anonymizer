...
- **Built-in regex recognition is on by default** for `credit_debit_card`, `email`, `ipv4`, `ipv6`, `mac_address`, and `url` whenever those labels are in scope. Users normally omit `builtin_regexes=True`; use `builtin_regexes=False` to disable all built-ins. The single `regex_rules` list accepts `BuiltinRegex(label="email", enabled=False)` to configure one built-in and `RegexRule(...)` for custom patterns. Both default `validate_with_llm` to `True`. Set `regex_only=True` on an enabled rule to make regex recognition authoritative for that label: it disables contextual LLM validation and removes the label from GLiNER and LLM augmentation.
