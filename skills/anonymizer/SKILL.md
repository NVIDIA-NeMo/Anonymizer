...
- **`Detect.deterministic_detection=True` is experimental and opt-in.** It runs a local first pass for structured labels such as email, SSN, URL, IP address, MAC address, and credit/debit card, then sends the remaining labels to GLiNER. Leave it unset unless the user asks to evaluate the split-pole path or their data is heavy with those structured identifiers.
