...
- **`Detect(gliner_only=True)` is an explicit speed/quality trade-off.** It skips LLM validation and augmentation, so use it only when the user accepts more false positives and false negatives. It does not make a remote detector local; local-only execution also requires a local `entity_detector` endpoint and a non-LLM replacement strategy.
