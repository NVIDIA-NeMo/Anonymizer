...
def test_validate_model_alias_references_raises_on_unknown_replace_alias_when_enabled(
    ...
    selected_models = stub_slim_model_selection.model_copy(
        update={"replace": ReplaceModelSelection(replacement_generator="bad-replace-alias", detection_judge="known", type_fidelity_judge="known", relational_consistency_judge="known")}
    )
