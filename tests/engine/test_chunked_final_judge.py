...
def test_aggregate_judge_takes_min_per_dimension():
    agg = aggregate_judge([_scores("high", "high", "medium", "good"), _scores("low", "medium", "high", "leak")])
    assert agg["privacy"]["score"] == "low"
    assert agg["quality"]["score"] == "medium"
    assert agg["style"]["score"] == "medium"
...
def test_judge_row_fast_path_single_call():
    facade = FakeJudgeFacade(_scores("high", "high", "high").model_dump())
    ...
    assert set(ev) == {"privacy", "quality", "style"}
    assert ev["privacy"]["score"] == "high"


def test_judge_row_windowed_aggregates_min():
    # distinct score per window so we can verify worst-window aggregation
    import itertools
    ...
    def resp(_prompt):
        i = next(counter)
        return _scores("low" if i == 0 else "high", "high", "high").model_dump()
    ...
    assert len(facade.calls) > 1
    # worst privacy across windows is the first window's "low"
    assert row[COL_JUDGE_EVALUATION]["privacy"]["score"] == "low"
...
def test_judge_row_skips_failing_window_not_fatal():
    ...
    def resp(_prompt):
        ...
        return _scores("medium", "medium", "medium").model_dump()
    ...
    assert row[COL_JUDGE_EVALUATION]["privacy"]["score"] == "medium"
