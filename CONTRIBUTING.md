...
6. Install the repository hooks once per clone with `mise run install-pre-commit`.
...
verify `uv.lock` when `pyproject.toml` changes, and run `mise run check`. The aggregate check covers four read-only stages:
